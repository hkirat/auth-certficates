...
const GenerateCertificate = dynamic(
  () =>
    import('@/components/GenerateCertificate/GenerateCertificate').then(
  ...
  useEffect(() => {
    const loadCertificate = async () => {
      try {
        ...
        const createdCertifcate = await apiLoadCertificateWithNumer(
          params?.id as string
        )
        ...
        toast.error(
          typeof errorData === 'string' ? errorData : 'Something went wround'
        )
      } finally {
